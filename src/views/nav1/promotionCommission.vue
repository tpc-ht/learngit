          ...
          <!-- <el-table 
			...
			<el-table-column
                ...
                <el-table-column label="操作" width="100">
                    <template slot-scope="scope">
                        <el-button @click="dialogForm(scope)" type="text" size="small" v-if="scope.row.status==1">打款</el-button>
                        <!--<el-button @click="toAssociatedOrder(scope)" type="text" size="small">查看详情</el-button>-->
                    </template>
                </el-table-column>
            </el-table>
        ...
        methods: {
            ...
            dialogForm(scope){
                this.$confirm('确定要打款吗？', '提示', {
                    ...
                    this.axios({
                        method: "post",
                        url: this.$local.http + this.$api.confirmationPayment,
                        data: qs.stringify(data),
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        }
			 ...
			 toggleSelection(row) {
				...
				formDatas.append('batchyRoyaltyIdList',JSON.stringify(this.tableDataList));
				this.axios({
				    method: "post",
				    url: this.$local.http + this.$api.updateBatchRoyalty,
				    data: formDatas,
				}).then(res => {
				    if (res.data.success) {
				        this.$message({message: res.data.msg, type: 'success'});
