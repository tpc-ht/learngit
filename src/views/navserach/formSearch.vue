<template>
	<div style="padding:20px;">
		...
		<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
			...
			<el-tab-pane label="下下级代理" name="second">
				<div class="show_two">
				<div class="head-ed">
					<div class="inputes">
						<div class="block">
				...
				<div>
					...
					<el-table :data="tableList.list" border style="width: 100%">
						...
						<el-table-column prop="proxy_status" label="所属上级代理" width="500">
							<template slot-scope="scope">
								<span>{{scope.row.superRealName}}</span>
								<span style="padding-left: 10px;">{{scope.row.superPhone}}</span>
								<p>{{scope.row.superProxyRegion}}
									<span style="padding-left: 10px;">
							<span v-if="scope.row.superLevel==1">省级代理商</span>
							<span v-else-if="scope.row.superLevel==2">市级代理商</span>
							<span v-else-if="scope.row.superLevel==3">县级代理商</span>
							<span v-else-if="scope.row.superLevel==4">乡级代理商</span>
							<span v-else-if="scope.row.superLevel==5">村级代理商</span></span></p>
							</template>
						</el-table-column>
						<el-table-column prop="proxyMoney" label="代理费提成" width="100">
							<template slot-scope="scope">
				...
				<!-- 分页 -->
				<pagination v-show="tableList.total>0" :total="tableList.total" :page.sync="form.page" :limit.sync="form.limit"
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeTimerData(){
				this.dataTime2= this.timerData[0].getTime();
				this.dataTime3= this.timerData[1].getTime();
			},
			getList() { //查询
				let data = {
					startDate: this.dataTime,
					endDate: this.dataTime1,
					agentParentId:this.id,//父级ID（和父级的父级ID选传一个）
					agentParentParentId:'',//父级的父级ID（和父级ID选传一个）
					level:this.level,
				...
				this.axios({
					method: "post",
					url: this.$local.http + this.$api.getLowerAgentInfo,
			...
			getLowerAgentData() { //查询
				let data = {
					agentParentId:'',//父级ID（和父级的父级ID选传一个）
					agentParentParentId:this.id,//父级的父级ID（和父级ID选传一个）
					startDate:this.dataTime2,//省
					endDate:this.dataTime3,//县（区）
					level:this.level,
					province:this.province,
					city:this.city,
					country:this.country,
				...
				this.axios({
					method: "post",
					url: this.$local.http + this.$api.getLowerAgentInfo2,
					data: qs.stringify(data),
					headers: {
						'Content-Type': 'application/x-www-form-urlencoded'
					}
