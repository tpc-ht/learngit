<template>
	<div style="padding:20px;">
		<div class="head-ed">
			...
			<span class="se-pl" style="margin-left:26px;">结款状态</span>
		...
		<div>
			...
			<el-table :data="tableData.list" border style="width: 100%">
				...
				<el-table-column prop="proxy_region" label="代理区域" width="500">
				</el-table-column>
				<el-table-column prop="typeid" label="代理类型" width="80">
				...
				<el-table-column prop="end_time" label="代理时间" width="190">
					<template slot-scope="scope">
				...
				<el-table-column prop="proxyDevPay" label="区域商家代理费提成" width="150">
					<template slot-scope="scope">
						<span>￥{{scope.row.proxyDevPay||0}}</span>
					</template>
				</el-table-column>
				<el-table-column prop="proxyBusinessPay" label="自主开发商家提成" width="140">
					<template slot-scope="scope">
						<span>￥{{scope.row.proxyBusinessPay||0}}</span>
					</template>
				</el-table-column>
				...
				<el-table-column prop="pay_money" label="实际结款" width="80">
					<template slot-scope="scope">
						<span>￥{{scope.row.pay_money||0}}</span>
					</template>
				</el-table-column>
				<el-table-column prop="finalStatus" label="结款状态" width="80">
					<template slot-scope="scope">
	...
	export default {
		...
		methods: {
			...
			getList() { //初始化
				...
				this.axios({
					method: "post",
					url: this.$local.http + this.$api.queryAgentInfoByBusinessPay,
			...
			djClick(scope) { //查看详情
				this.$router.push({path:'/tanSearch',query:{real_name:scope.row.real_name,phone:scope.row.phone,
						...
						proxyMoney:scope.row.proxyMoney,pay_money:scope.row.pay_money,
						province:scope.row.province,
						city:scope.row.city,
						country:scope.row.country,
						town:scope.row.town,
						village:scope.row.village,
						typeid:scope.row.typeid,
						shopTypeId:scope.row.shopTypeId,
					}})
			},
			getYw() { //查询业务
				this.axios({
